...
fun <-
function(runid=NA, q=1, distr="P", overlap=FALSE)
{
    ...
    N <- if (distr=="P") {
        ...
        rnbinom(n, size=1/gvar, prob=1/(1+gvar*lambda))
    }
    Y <- rbinom(n, N, delta)
    kk <- max(K, max(N)+50)

    m1 <- svabu_nb.fit(Y, X, Z, Q = NULL,
        zeroinfl = FALSE, area = 1, N.max = kk,
        link.det = "logit", link.zif = "logit")
    m2 <- svabu_nb2.fit(Y, X, Z, Q = NULL,
        zeroinfl = FALSE, area = 1, N.max = kk,
        link.det = "logit", link.zif = "logit")
    ...
    lam2 <- mean(exp(drop(X %*% cf2[1:ncol(X)])))
    p0 <- mean(p)
    p1 <- mean(linkinv(drop(Z %*% cf1[(ncol(X)+1):(ncol(X)+ncol(Z))])))
    p2 <- mean(linkinv(drop(Z %*% cf2[(ncol(X)+1):(ncol(X)+ncol(Z))])))

    out <- cbind(
        truth=c(lam=lam0, p=p0, q=q, coef=cf0, log.gvar=log(gvar)),
        NBp   =c(lam1, p1, NA, c(cf1, NA), m1$var$est),
        NBpq   =c(lam2, p2, plogis(cf2[length(cf2)]), cf2, m2$var$est))
    ...
    attr(out, "AIC_NBp") <- -2*m1$loglik + 2*length(cf1)
    attr(out, "AIC_NBpq") <- -2*m2$loglik + 2*length(cf2)
    out
}
