...
# WOSP = Woodpecker Spp = unidentified woodpecker = UNWO
x$SPECIES[x$SPECIES == "WOSP"] <- "UNWO"
# BADO = Barred Owl=BDOW
x$SPECIES[x$SPECIES == "BADO"] <- "BDOW"
...
# BCFR = Boreal Chorus Frog - not a bird
# WOFR = Wood Frog - not a bird

s <- read.csv("data-raw/paired/aou-codes.csv")
s <- rbind(s,
    data.frame(
        id = c("UNKN", "RESQ", "BCFR", "WOFR"), 
        name = c("Unknown", "Red Squirrel", "Boreal Chorus Frog", "Wood Frog")))
